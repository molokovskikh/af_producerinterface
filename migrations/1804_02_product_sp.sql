DROP PROCEDURE IF EXISTS quartz.ProductRatingReport;
CREATE DEFINER=`RootDBMS`@`127.0.0.1` PROCEDURE quartz.`ProductRatingReport`(IN `CatalogId` VARCHAR(255), IN `RegionCode` VARCHAR(255), IN `SupplierId` VARCHAR(255), IN `DateFrom` datetime, IN `DateTo` datetime)
BEGIN
