...
CREATE VIEW `catalognames` AS 
select `c`.`Id` AS `CatalogId`,
concat(`cn`.`Name`,' ',`cf`.`Form`) AS `CatalogName`,
`c`.`Pharmacie` AS `IsPharmacie` 
...
CREATE VIEW `producernames` AS 
select `p`.`Id` AS `ProducerId`,
`p`.`Name` AS `ProducerName` 
from `catalogs`.`producers` `p`;

CREATE VIEW `regionnames` AS 
select `r`.`RegionCode` AS `RegionCode`,
`r`.`Region` AS `RegionName` 
from `farm`.`regions` `r`;

CREATE VIEW `suppliernames` AS 
select `s`.`Id` AS `SupplierId`,concat(`s`.`Name`,' - ',`rg`.`Region`) AS `SupplierName` 
